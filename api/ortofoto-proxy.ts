...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const targetUrl = `${ORTOFOTO_TARGET_URL}?${queryString}`;
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`Ortofoto proxy error: ${response.status}`);
    ...
    console.error('Ortofoto Proxy Error:', error instanceof Error ? error.message : 'Unknown error');
